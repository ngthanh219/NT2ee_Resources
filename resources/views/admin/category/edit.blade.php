        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <br>
                        <form method="POST" action="{{ route('categories.update', $category->id) }}" id="demo-form2" class="form-horizontal">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="col-md-9 col-sm-9 ">
                                    <select class="form-control" name="view">
                                        <option value="{{ config('base.view.show') }}"
                                            {{ $category->view == config('base.view.show') ? 'selected' : '' }}>Hiển thị
                                        </option>
                                        <option value="{{ config('base.view.hidden') }}"
                                            {{ $category->view == config('base.view.hidden') ? 'selected' : '' }}>Ẩn</option>
                                    </select>
                                    @if ($errors->has('view'))
