        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <br>
                        <form method="POST" action="{{ route('posts.update', $post->id) }}" id="form" class="form-horizontal" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="col-md-9 col-sm-9 ">
                                    <select class="form-control" name="view">
                                        <option value="{{ config('base.view.show') }}"
                                            {{ $post->view == config('base.view.show') ? 'selected' : '' }}>Hiển thị
                                        </option>
                                        <option value="{{ config('base.view.hidden') }}"
                                            {{ $post->view == config('base.view.hidden') ? 'selected' : '' }}>Ẩn</option>
                                    </select>
                                    @if ($errors->has('view'))
                                ...
                                <div class="img-list">
                                    <div class="img-item">
                                        ...
                                        <div class="img-action">
                                            <a href="{{ asset('images/' . $post->image) }}" target="__blank">Xem</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
